...
#ifndef CORSAC_TYPE_POD_H
#define CORSAC_TYPE_POD_H
...
namespace corsac
{
    ...
    // Наследование в empty_helper_t1 не будет работать с неклассовыми типами.
    template <typename T, bool is_a_class = false>
    struct is_empty_helper : public corsac::false_type{};

    template <typename T>
    struct is_empty_helper<T, true> : public corsac::integral_constant<bool,
    ...
    {
        using cvT = typename corsac::remove_cv<T>::type;
        using type = corsac::is_empty_helper<cvT, corsac::is_class<cvT>::value>;
    };

    template <typename T>
    struct is_empty : public corsac::is_empty_helper2<T>::type {};
    ...
    * что встроенные типы, такие как int, являются POD. Вы можите помочь
    * используя макрос EASTL_DECLARE_POD в классе.
    */
    ...
    #elif CORSAC_COMPILER_INTRINSIC_TYPE_TRAITS_AVAILABLE && (defined(EA_COMPILER_GNUC) || (defined(__clang__) && CORSAC_COMPILER_HAS_FEATURE(is_pod)))
