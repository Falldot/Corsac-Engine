#ifndef CORSAC_FILL_HELP_H
#define CORSAC_FILL_HELP_H
...
namespace corsac
{
    // fill_imp
    //
    // Мы реализуем некоторые вспомогательные функции заполнения, чтобы мы могли оптимизировать его, где это возможно.
    //
    ...
    struct fill_imp
    {
        ...
        {
            // Стандарт C++ не указывает, нужно ли нам создавать временное или нет, но все реализации std STL написаны так, как здесь.
    ...
    struct fill_imp<true>
    {
        ...
        static void do_fill(ForwardIterator first, ForwardIterator last, const T& value)
        {
            ...
            // Мы создаем временный интервал и заполняем его, потому что
            // значение может быть псевдонимом целевого диапазона, и
            // поэтому компилятор будет вынужден генерировать менее
            // эффективный код.
    ...
    {
        corsac::fill_imp<is_scalar<T>::value >::do_fill(first, last, value);
...
//        const bool bEasyCopy = corsac::type_and<corsac::has_trivial_constructor<T>::value,
//                                               corsac::integral_constant<bool, (sizeof(T) <= 16)>::value;
//        corsac::fill_imp<bEasyCopy>::do_fill(first, last, value);
    ...
    {
        return corsac::fill_n_imp<is_scalar<T>::value>::do_fill(first, n, value);
    }
    ...
    #if (defined(EA_COMPILER_GNUC) || defined(__clang__)) && (defined(CORSAC_PROCESSOR_X86) || defined(CORSAC_PROCESSOR_X86_64))
        #if defined(EA_PROCESSOR_X86_64)
