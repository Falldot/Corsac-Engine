...
* обновляеться с учетом последних версий библиотек EASTL и Boost, а также в зависимости от
...
*
*    alignment_of<T>                           Целочисленное значение, представляющее количество байтов выравнивания объектов типа T; объект типа T может быть размещен по адресу, кратному его выравниванию.
*    rank<T>                                   Целочисленное значение, представляющее ранг объектов типа T. Термин "ранг" здесь используется для описания количества измерений типа массива.
*    extent<T, i>                              Целочисленное значение, представляющее протяженность (размерность) i-й границы объектов типа T. Если тип T не является типом массива, имеет ранг меньше I или если I == 0 и T имеет тип ' массив неизвестной границы U, тогда значение должно быть равно нулю; в противном случае value будет оцениваться как количество элементов в i-й границе массива T. Термин «степень» здесь используется для описания количества элементов в типе массива.
*
...
*    is_reference_wrapper                      <STL/functional.h>
