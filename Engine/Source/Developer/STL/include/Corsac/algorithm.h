...
#ifndef CORSAC_STL_ALGORITHM_H
#define CORSAC_STL_ALGORITHM_H
...
#include "Corsac/STL/help_copy.h"
#include "Corsac/STL/help_fill.h"
...
namespace corsac
{
    ...
    * min_element
    *
    * min_element находит наименьший элемент в диапазоне [первый, последний).
    * Он возвращает первый итератор i в [first, last), так что никакой другой
    * итератор в [first, last) не указывает на значение меньше *i.
    * Возвращаемое значение является последним, если и только если [first, last)
    * является пустым диапазоном.
    *
    * Возвращает: первый итератор i в диапазоне [first, last) такой,
    * что для любого итератора j в диапазоне [first, last) выполняется следующее
    *  соответствующее условие:! (*J < *i).
    *
    * Сложность: Ровно max ((last - first) - 1, 0) приложений соответствующих сравнений.
    */
    ...
    ForwardIterator max_element(ForwardIterator first, ForwardIterator last, Compare compare)
    {
		...
		///
		template <typename T>
		inline constexpr typename corsac::enable_if<corsac::is_scalar<T>::value, T>::type
		min(T a, T b)
		{
			return b < a ? b : a;
		}
		...
		template <typename T>
		inline constexpr typename corsac::enable_if<!corsac::is_scalar<T>::value, const T&>::type
		min(const T& a, const T& b)
		{
			return b < a ? b : a;
		}

		inline constexpr float       min(float  a, float   b)           { return b < a ? b : a; }
		inline constexpr double      min(double a, double  b)           { return b < a ? b : a; }
		inline constexpr long double min(long double a, long double  b) { return b < a ? b : a; }
    #endif // CORSAC_MINMAX_ENABLED
    ...
    template <typename T>
    inline typename corsac::enable_if<!corsac::is_scalar<T>::value, const T&>::type
    ...
    /// max
        ...
        inline constexpr float       max(float       a, float       b) { return a < b ? b : a; }
        inline constexpr double      max(double      a, double      b) { return a < b ? b : a; }
        inline constexpr long double max(long double a, long double b) { return a < b ? b : a; }
#endif // CORSAC_MINMAX_ENABLED
    ...
    {
        return *corsac::min_element(ilist.begin(), ilist.end());
    }
    ...
    {
        return *corsac::min_element(ilist.begin(), ilist.end(), compare);
    }
    ...
    {
        return *corsac::max_element(ilist.begin(), ilist.end());
    }
    ...
    {
        return *corsac::max_element(ilist.begin(), ilist.end(), compare);
    }
    ...
    ///
    template <typename ForwardIterator, typename Compare>
    corsac::pair<ForwardIterator, ForwardIterator>
    minmax_element(ForwardIterator first, ForwardIterator last, Compare compare)
    {
        corsac::pair<ForwardIterator, ForwardIterator> result(first, first);
    ...
    minmax_element(ForwardIterator first, ForwardIterator last)
    {
        ...
        return corsac::minmax_element(first, last, corsac::less<value_type>());
    }
    ...
    //
    // template <typename T>
    // inline constexpr typename corsac::enable_if<corsac::is_scalar<T>::value, corsac::pair<T, T> >::type
    // minmax(T a, T b)
    // {
    //     return (b < a) ? corsac::make_pair(b, a) : corsac::make_pair(a, b);
    // }
    //
    // template <typename T>
    // inline typename corsac::enable_if<!corsac::is_scalar<T>::value, corsac::pair<const T&, const T&> >::type
    ...
    template <typename T>
    inline corsac::pair<const T&, const T&>
    minmax(const T& a, const T& b)
    {
        return (b < a) ? corsac::make_pair(b, a) : corsac::make_pair(a, b);
    }
    ...
    template <typename T, typename Compare>
    corsac::pair<const T&, const T&>
    minmax(const T& a, const T& b, Compare compare)
    {
        return compare(b, a) ? corsac::make_pair(b, a) : corsac::make_pair(a, b);
    }
    ...
    minmax(std::initializer_list<T> ilist)
    {
        ...
        corsac::pair<iterator_type, iterator_type> iteratorPair = corsac::minmax_element(ilist.begin(), ilist.end());
        return corsac::make_pair(*iteratorPair.first, *iteratorPair.second);
    }
    ...
    {
        typedef typename std::initializer_list<T>::iterator iterator_type;
        corsac::pair<iterator_type, iterator_type> iteratorPair = corsac::minmax_element(ilist.begin(), ilist.end(), compare);
        return corsac::make_pair(*iteratorPair.first, *iteratorPair.second);
    }
    ...
    inline T&& median_impl(T&& a, T&& b, T&& c)
    {
        ...
        {
            if(b < c)
                return corsac::forward<T>(b);
            else if(a < c)
                return corsac::forward<T>(c);
            else
                return corsac::forward<T>(a);
        }
        else if(a < c)
            return corsac::forward<T>(a);
        else if(b < c)
    ...
    {
        return corsac::forward<T>(median_impl(corsac::forward<T>(a), corsac::forward<T>(b), corsac::forward<T>(c)));
    }
    ...
    {
        return corsac::forward<T>(median_impl<T&&, Compare>(corsac::forward<T>(a), corsac::forward<T>(b), corsac::forward<T>(c), compare));
    }
    ...
    void shuffle(RandomAccessIterator first, RandomAccessIterator last, UniformRandomNumberGenerator&& urng)
    {
        if(first != last)
        {
            using difference_type = typename corsac::iterator_traits<RandomAccessIterator>::difference_type;
            using unsigned_difference_type = typename corsac::make_unsigned<difference_type>::type;
            using uniform_int_distribution = typename corsac::uniform_int_distribution<unsigned_difference_type>;
    ...
    {
        typedef typename corsac::iterator_traits<RandomAccessIterator>::difference_type difference_type;
    ...
    move_n_impl(InputIterator first, Size n, OutputIterator result, corsac::input_iterator_tag)
    {
        for(; n > 0; --n)
    ...
    inline OutputIterator
    move_n_impl(RandomAccessIterator first, Size n, OutputIterator result, corsac::random_access_iterator_tag)
    {
        return corsac::move(first, first + n, result); // Take advantage of the optimizations present in the move algorithm.
    }
    ...
    {
        typedef typename corsac::iterator_traits<InputIterator>::iterator_category IC;
        return corsac::move_n_impl(first, n, result, IC());
    }
    ...
    template <typename InputIterator, typename Size, typename OutputIterator>
    inline OutputIterator
    copy_n_impl(InputIterator first, Size n, OutputIterator result, corsac::input_iterator_tag)
    {
        for(; n > 0; --n)
            *result++ = *first++;
        return result;
    ...
    template <typename RandomAccessIterator, typename Size, typename OutputIterator>
    inline OutputIterator
    copy_n_impl(RandomAccessIterator first, Size n, OutputIterator result, corsac::random_access_iterator_tag)
    {
        return corsac::copy(first, first + n, result); // Take advantage of the optimizations present in the copy algorithm.
    }
    ...
    {
        typedef typename corsac::iterator_traits<InputIterator>::iterator_category IC;
        return corsac::copy_n_impl(first, n, result, IC());
    }
    ...
    template<>
    struct move_and_copy_backward_helper<corsac::random_access_iterator_tag, true, false>
    {
        template<typename BidirectionalIterator1, typename BidirectionalIterator2>
        static BidirectionalIterator2 move_or_copy_backward(BidirectionalIterator1 first, BidirectionalIterator1 last, BidirectionalIterator2 resultEnd)
        {
            typedef typename corsac::iterator_traits<BidirectionalIterator1>::difference_type difference_type;
    ...
    template <>
    struct move_and_copy_backward_helper<corsac::random_access_iterator_tag, false, false>
    {
        template <typename BidirectionalIterator1, typename BidirectionalIterator2>
        static BidirectionalIterator2 move_or_copy_backward(BidirectionalIterator1 first, BidirectionalIterator1 last, BidirectionalIterator2 resultEnd)
        {
    ...
    struct move_and_copy_backward_helper<corsac::random_access_iterator_tag, isMove, true>
    {
        template <typename T>
    ...
    {
        typedef typename corsac::iterator_traits<BidirectionalIterator1>::iterator_category IIC;
        typedef typename corsac::iterator_traits<BidirectionalIterator2>::iterator_category OIC;
        typedef typename corsac::iterator_traits<BidirectionalIterator1>::value_type        value_type_input;
        typedef typename corsac::iterator_traits<BidirectionalIterator2>::value_type        value_type_output;
        ...
        const bool canBeMemmoved = corsac::is_trivially_copyable<value_type_output>::value &&
                                   ...
                                   (corsac::is_pointer<BidirectionalIterator1>::value || corsac::is_same<IIC, corsac::contiguous_iterator_tag>::value) &&
                                   (corsac::is_pointer<BidirectionalIterator2>::value || corsac::is_same<OIC, corsac::contiguous_iterator_tag>::value);

        return corsac::move_and_copy_backward_helper<IIC, isMove, canBeMemmoved>::move_or_copy_backward(first, last, resultEnd); // Need to chose based on the input iterator tag and not the output iterator tag, because containers accept input ranges of iterator types different than self.
    }
    ...
    {
        return BidirectionalIterator2(corsac::move_and_copy_backward_chooser<isMove>(corsac::unwrap_iterator(first), corsac::unwrap_iterator(last), corsac::unwrap_iterator(resultEnd))); // Have to convert to BidirectionalIterator2 because result.base() could be a T*
    }
    ...
    */
    ...
    {
        return corsac::move_and_copy_backward_unwrapper<true>(corsac::unwrap_iterator(first), corsac::unwrap_iterator(last), resultEnd);
    }
    ...
    {
        const bool isMove = corsac::is_move_iterator<BidirectionalIterator1>::value; CORSAC_UNUSED(isMove);

        return corsac::move_and_copy_backward_unwrapper<isMove>(corsac::unwrap_iterator(first), corsac::unwrap_iterator(last), resultEnd);
    }
    ...
    inline typename corsac::iterator_traits<InputIterator>::difference_type
    count(InputIterator first, InputIterator last, const T& value)
    {
        typename corsac::iterator_traits<InputIterator>::difference_type result = 0;
                      ...
                      ForwardIterator2 first2, ForwardIterator2 last2)
    {
        for(; first1 != last1; ++first1)
        {
            if(corsac::find(first2, last2, *first1) == last2)
                break;
        }
    ...
    find_first_not_of(ForwardIterator1 first1, ForwardIterator1 last1,
                      ...
                      BinaryPredicate predicate)
    {
        typedef typename corsac::iterator_traits<ForwardIterator1>::value_type value_type;

        for(; first1 != last1; ++first1)
        {
            if(corsac::find_if(first2, last2, corsac::bind1st<BinaryPredicate, value_type>(predicate, *first1)) == last2)
                break;
        }
                 ...
                 ForwardIterator2 first2, ForwardIterator2 last2)
    {
        if((first1 != last1) && (first2 != last2))
        {
            ...
            while((--it1 != first1) && (corsac::find(first2, last2, *it1) == last2))
                ; // Do nothing

            if((it1 != first1) || (corsac::find(first2, last2, *it1) != last2))
                return it1;
        }
    ...
    find_last_of(BidirectionalIterator1 first1, BidirectionalIterator1 last1,
                 ...
                 BinaryPredicate predicate)
    {
        typedef typename corsac::iterator_traits<BidirectionalIterator1>::value_type value_type;

        if((first1 != last1) && (first2 != last2))
        {
            BidirectionalIterator1 it1(last1);

            while((--it1 != first1) && (corsac::find_if(first2, last2, corsac::bind1st<BinaryPredicate, value_type>(predicate, *it1)) == last2))
                ; // Do nothing

            if((it1 != first1) || (corsac::find_if(first2, last2, corsac::bind1st<BinaryPredicate, value_type>(predicate, *it1)) != last2))
                return it1;
        }
    ...
    constexpr inline InputIterator
    ...
    lexicographical_compare(const char* first1, const char* last1, const char* first2, const char* last2)
    {
        ...
        const int result = memcmp(first1, first2, (size_t)corsac::min_alt(n1, n2));
    ...
    inline corsac::pair<InputIterator1, InputIterator2>
             ...
             InputIterator2 first2) // , InputIterator2 last2)
    {
        ...
        return corsac::pair<InputIterator1, InputIterator2>(first1, first2);
    }
    ...
    {
        typedef typename corsac::iterator_traits<ForwardIterator>::difference_type DifferenceType;

        DifferenceType d = corsac::distance(first, last); // This will be efficient for a random access iterator such as an array.
        ...
        while(d > 0)
        {
            ...
            corsac::advance(i, d2); // This will be efficient for a random access iterator such as an array.

            if(*i < value)
            {
                // Disabled because std::lower_bound doesn't specify (23.3.3.3, p3) this can be done: CORSAC_VALIDATE_COMPARE(!(value < *i)); // Validate that the compare function is sane.
                first = ++i;
                d    -= d2 + 1;
            }
            else
    ...
    lower_bound(ForwardIterator first, ForwardIterator last, const T& value, Compare compare)
    {
        ...
        while(d > 0)
        {
            ...
            if(compare(*i, value))
            {
                // Disabled because std::lower_bound doesn't specify (23.3.3.1, p3) this can be done: CORSAC_VALIDATE_COMPARE(!compare(value, *i)); // Validate that the compare function is sane.
                first = ++i;
                d    -= d2 + 1;
            }
            else
    ...
    upper_bound(ForwardIterator first, ForwardIterator last, const T& value)
    {
        ...
        DifferenceType len = corsac::distance(first, last);
        ...
        while(len > 0)
        {
            ...
            {
                // Disabled because std::upper_bound doesn't specify (23.3.3.2, p3) this can be done: CORSAC_VALIDATE_COMPARE(!(*i < value)); // Validate that the compare function is sane.
                len = len2;
            }
        }
        return first;
    ...
    upper_bound(ForwardIterator first, ForwardIterator last, const T& value, Compare compare)
    {
        ...
        while(len > 0)
        {
            ...
            {
                // Disabled because std::upper_bound doesn't specify (23.3.3.2, p3) this can be done: CORSAC_VALIDATE_COMPARE(!compare(*i, value)); // Validate that the compare function is sane.
                len = len2;
            }
        }
        return first;
    ...
    equal_range(ForwardIterator first, ForwardIterator last, const T& value)
    {
        ...
        DifferenceType d = corsac::distance(first, last);
        ...
        while(d > 0)
        {
            ...
            if(*i < value)
            {
                CORSAC_VALIDATE_COMPARE(!(value < *i)); // Validate that the compare function is sane.
                first = ++i;
                d    -= d2 + 1;
            }
            else if(value < *i)
            {
                CORSAC_VALIDATE_COMPARE(!(*i < value)); // Validate that the compare function is sane.
                d    = d2;
                last = i;
            }
            else
            {
                ...
                return ResultType(corsac::lower_bound(first, i, value),
                                  corsac::upper_bound(++j, last, value));
            }
        }
    ...
    equal_range(ForwardIterator first, ForwardIterator last, const T& value, Compare compare)
    {
        ...
        while(d > 0)
        {
            ...
            if(compare(*i, value))
            {
                CORSAC_VALIDATE_COMPARE(!compare(value, *i)); // Validate that the compare function is sane.
                first = ++i;
                d    -= d2 + 1;
            }
            else if(compare(value, *i))
            {
                CORSAC_VALIDATE_COMPARE(!compare(*i, value)); // Validate that the compare function is sane.
                d    = d2;
                last = i;
            }
            else
            {
                ...
                return ResultType(corsac::lower_bound(first, i, value, compare),
                                  corsac::upper_bound(++j, last, value, compare));
            }
        }
        return ResultType(first, first);
    }
    ...
    {
        first = corsac::find(first, last, value);
        if(first != last)
        {
            ForwardIterator i(first);
            return corsac::remove_copy(++i, last, first, value);
        }
        return first;
    }
    ...
    {
        first = corsac::find_if(first, last, predicate);
        if(first != last)
        {
            ForwardIterator i(first);
            return corsac::remove_copy_if<ForwardIterator, ForwardIterator, Predicate>(++i, last, first, predicate);
        }
        return first;
    }
    ...
    inline void reverse_impl(BidirectionalIterator first, BidirectionalIterator last, corsac::bidirectional_iterator_tag)
    {
        for(; (first != last) && (first != --last); ++first) // We are not allowed to use operator <, <=, >, >= with a
            corsac::iter_swap(first, last);                   // generic (bidirectional or otherwise) iterator.
    }

    template <typename RandomAccessIterator>
    inline void reverse_impl(RandomAccessIterator first, RandomAccessIterator last, corsac::random_access_iterator_tag)
    {
        if(first != last)
        {
            for(; first < --last; ++first)      // With a random access iterator, we can use operator < to more efficiently implement
                corsac::iter_swap(first, last);  // this algorithm. A generic iterator doesn't necessarily have an operator < defined.
        }
    }
    ...
    {
        typedef typename corsac::iterator_traits<BidirectionalIterator>::iterator_category IC;
        corsac::reverse_impl(first, last, IC());
    }
           ...
           ForwardIterator2 first2, ForwardIterator2 last2)
    {
        if(first2 != last2) // If there is anything to search for...
        {
            ...
            if(temp2 != last2) // If what we are searching for has a length > 1...
            {
                ...
                while(first1 != last1)
                {
                    // The following loop is the equivalent of corsac::find(first1, last1, *first2)
                ...
                return corsac::find(first1, last1, *first2);
        }
        ...
        /*  Another implementation which is a little more simpler but executes a little slower on average.
                typedef typename corsac::iterator_traits<ForwardIterator1>::difference_type difference_type_1;
                typedef typename corsac::iterator_traits<ForwardIterator2>::difference_type difference_type_2;

                const difference_type_2 d2 = corsac::distance(first2, last2);

                for(difference_type_1 d1 = corsac::distance(first1, last1); d1 >= d2; ++first1, --d1)
                {
           ...
           BinaryPredicate predicate)
    {
        ...
        difference_type_2 d2 = corsac::distance(first2, last2);
        ...
        if(d2 != 0)
        {
            ...
            for(difference_type_1 d1 = corsac::distance(first1, last1); d1 >= d2; --d1)
            {
                if(corsac::equal<ForwardIterator1, ForwardIterator2, BinaryPredicate>(first1, i, first2, predicate))
                    return first1;
    ...
    search_n_impl(ForwardIterator first, ForwardIterator last, Size count, const T& value, corsac::forward_iterator_tag)
    {
        if(count <= 0)
            return first;

        Size d1 = (Size)corsac::distance(first, last); // Should d1 be of type Size, ptrdiff_t, or iterator_traits<ForwardIterator>::difference_type?
    ...
    search_n_impl(RandomAccessIterator first, RandomAccessIterator last, Size count, const T& value, corsac::random_access_iterator_tag)
    {
        if(count <= 0)
            return first;
        else if(count == 1)
            return corsac::find(first, last, value);
        else if(last > first)
        {
    ...
    {
        typedef typename corsac::iterator_traits<ForwardIterator>::iterator_category IC;
        return corsac::search_n_impl(first, last, count, value, IC());
    }
    ...
    binary_search(ForwardIterator first, ForwardIterator last, const T& value)
    {
        ...
        ForwardIterator i(corsac::lower_bound<ForwardIterator, T>(first, last, value));
    ...
    {
        // To do: This can be made slightly faster by not using lower_bound.
        ForwardIterator i(corsac::lower_bound<ForwardIterator, T, Compare>(first, last, value, compare));
    ...
    {
        first = corsac::adjacent_find<ForwardIterator>(first, last);
    ...
    {
        first = corsac::adjacent_find<ForwardIterator, BinaryPredicate>(first, last, predicate);
    ...
    find_end_impl(ForwardIterator1 first1, ForwardIterator1 last1,
                  ForwardIterator2 first2, ForwardIterator2 last2,
                  corsac::forward_iterator_tag, corsac::forward_iterator_tag)
    {
        if(first2 != last2) // We have to do this check because the search algorithm below will return first1 (and not last1) if the first2/last2 range is empty.
        {
            ...
            {
                const ForwardIterator1 resultNext(corsac::search(first1, last1, first2, last2));
    ...
    find_end_impl(BidirectionalIterator1 first1, BidirectionalIterator1 last1,
                  ...
                  corsac::bidirectional_iterator_tag, corsac::bidirectional_iterator_tag)
    {
        typedef corsac::reverse_iterator<BidirectionalIterator1> reverse_iterator1;
        typedef corsac::reverse_iterator<BidirectionalIterator2> reverse_iterator2;

        reverse_iterator1 rresult(corsac::search(reverse_iterator1(last1), reverse_iterator1(first1),
                                                 reverse_iterator2(last2), reverse_iterator2(first2)));
        if(rresult.base() != first1) // If we found something...
        {
            BidirectionalIterator1 result(rresult.base());

            corsac::advance(result, -corsac::distance(first2, last2)); // We have an opportunity to optimize this, as the
    ...
    find_end(ForwardIterator1 first1, ForwardIterator1 last1,
             ForwardIterator2 first2, ForwardIterator2 last2)
    {
        typedef typename corsac::iterator_traits<ForwardIterator1>::iterator_category IC1;
        typedef typename corsac::iterator_traits<ForwardIterator2>::iterator_category IC2;

        return corsac::find_end_impl(first1, last1, first2, last2, IC1(), IC2());
    }
                  ...
                  corsac::forward_iterator_tag, corsac::forward_iterator_tag)
    {
        if(first2 != last2) // We have to do this check because the search algorithm below will return first1 (and not last1) if the first2/last2 range is empty.
        {
            for(ForwardIterator1 result = last1; ; )
            {
                const ForwardIterator1 resultNext(corsac::search<ForwardIterator1, ForwardIterator2, BinaryPredicate>(first1, last1, first2, last2, predicate));
                  ...
                  corsac::bidirectional_iterator_tag, corsac::bidirectional_iterator_tag)
    {
        ...
        reverse_iterator1 rresult(corsac::search<reverse_iterator1, reverse_iterator2, BinaryPredicate>
        ...
        {
            BidirectionalIterator1 result(rresult.base());
            corsac::advance(result, -corsac::distance(first2, last2));
            return result;
        }
        return last1;
    }
             ...
             BinaryPredicate predicate)
    {
        ...
        return corsac::find_end_impl<ForwardIterator1, ForwardIterator2, BinaryPredicate>
                                  ...
                                  OutputIterator result)
    {
        ...
        return corsac::copy(first1, last1, result);
    }
                                  ...
                                  OutputIterator result, Compare compare)
    {
        while((first1 != last1) && (first2 != last2))
        {
            if(compare(*first1, *first2))
            {
                CORSAC_VALIDATE_COMPARE(!compare(*first2, *first1)); // Validate that the compare function is sane.
                *result = *first1;
                ++first1;
                ++result;
            }
            ...
            {
                CORSAC_VALIDATE_COMPARE(!compare(*first1, *first2)); // Validate that the compare function is sane.
                ++first2;
            }
            else
            {
                          ...
                          OutputIterator result1, OutputIterator result2, Compare compare)
    {
        ...
        corsac::copy(first2, last2, result2);
        corsac::copy(first1, last1, result1);
    }

    /// set_difference_2
    ///
    ///  set_difference_2 with the default comparison object is corsac::less<>.
    ///
    template <typename InputIterator1, typename InputIterator2, typename OutputIterator>
    void set_difference_2(InputIterator1 first1, InputIterator1 last1,
                          InputIterator2 first2, InputIterator2 last2,
    ...
    {
        corsac::set_difference_2(first1, last1, first2, last2, result1, result2, corsac::less<>{});
    }
                                            ...
                                            OutputIterator result)
    {
        ...
        return corsac::copy(first2, last2, corsac::copy(first1, last1, result));
    }
    ...
    /// set_decomposition
    ///
    ///  set_decomposition with the default comparison object is corsac::less<>.
    ///
    template <typename InputIterator1, typename InputIterator2,
    ...
    {
        return corsac::set_decomposition(first1, last1, first2, last2, result1, result2, result3, corsac::less<>{});
    }
    ...
    {
        typedef typename corsac::iterator_traits<ForwardIterator1>::difference_type difference_type;
        ...
        if(first1 != last1)
        {
            const difference_type first1Size = corsac::distance(first1, last1);
            ForwardIterator2 last2 = first2;
            corsac::advance(last2, first1Size);

            for(ForwardIterator1 i = first1; i != last1; ++i)
            {
                if(i == corsac::find(first1, i, *i))
                {
                    const difference_type c = corsac::count(first2, last2, *i);

                    if((c == 0) || (c != corsac::count(i, last1, *i)))
                        return false;
                }
            }
        }
    ...
    bool is_permutation(ForwardIterator1 first1, ForwardIterator1 last1, ForwardIterator2 first2, BinaryPredicate predicate)
    {
        ...
        if(first1 != last1)
        {
            ...
            for(ForwardIterator1 i = first1; i != last1; ++i)
            {
                if(i == corsac::find(first1, i, *i, predicate))
                {
                    const difference_type c = corsac::count(first2, last2, *i, predicate);

                    if((c == 0) || (c != corsac::count(i, last1, *i, predicate)))
                        return false;
                }
            }
        }
            ...
            if(first != --i) // If the range has more than one item...
            {
                for(;;)
                {
                    ...
                    if(compare(*--i, *ii)) // Find two consecutive values where the first is less than the second.
                    {
                        ...
                        {}
                        corsac::iter_swap(i, j);     // Swap the first and the final.
                        corsac::reverse(ii, last);   // Reverse the ranget from second to last.
                        return true;
                    }
    ...
    {
        typedef typename corsac::iterator_traits<BidirectionalIterator>::value_type value_type;

        return corsac::next_permutation(first, last, corsac::less<value_type>());
    }
    ...
    namespace internal
    {
        ...
        ForwardIterator move_rotate_left_by_one(ForwardIterator first, ForwardIterator last)
        {
            ...
            value_type temp(corsac::move(*first));
            ForwardIterator result = corsac::move(corsac::next(first), last, first);    // Обратите внимание, что, хотя наш тип шаблона - BidirectionalIterator, если фактический
            *result = corsac::move(temp);                                               // iterator - это RandomAccessIterator, тогда этот ход будет memmove для тривиальных типов.

            return result; // result указывает на последний элемент в диапазоне.
        }
        ...
        {
            using value_type = typename corsac::iterator_traits<BidirectionalIterator>::value_type;

            BidirectionalIterator beforeLast = corsac::prev(last);
            value_type temp(corsac::move(*beforeLast));
            BidirectionalIterator result = corsac::move_backward(first, beforeLast, last); // Note that while our template type is BidirectionalIterator, if the actual
            *first = corsac::move(temp);                                                   // iterator is a RandomAccessIterator then this move will be a memmove for trivial types.
        ...
        template <>
        struct rotate_helper<corsac::forward_iterator_tag, true>
        {
            template <typename ForwardIterator>
            static ForwardIterator rotate_impl(ForwardIterator first, ForwardIterator middle, ForwardIterator last)
            {
                if(corsac::next(first) == middle) // If moving trivial types by a single element, memcpy is fast for that case.
        ...
        template <>
        struct rotate_helper<corsac::bidirectional_iterator_tag, false>
        {
            template <typename BidirectionalIterator>
            ...
            /*
                ...
                static BidirectionalIterator rotate_impl(BidirectionalIterator first, BidirectionalIterator middle, BidirectionalIterator last)
                {
                    corsac::reverse(first, middle);
                    corsac::reverse(middle, last);
                    corsac::reverse(first, last);
                ...
                static BidirectionalIterator rotate_impl(BidirectionalIterator first, BidirectionalIterator middle, BidirectionalIterator last)
                {
                    ...
                    corsac::reverse_impl(first,  middle, corsac::bidirectional_iterator_tag()); // Reverse the left side.
                    corsac::reverse_impl(middle, last,   corsac::bidirectional_iterator_tag()); // Reverse the right side.

                    // Reverse the entire range.
                    while((first != middle) && (middle != last))
                    {
                        corsac::iter_swap(first, --last);
                        ++first;
                    }

                    if(first == middle) // Finish reversing the entire range.
                    {
                        corsac::reverse_impl(middle, last, bidirectional_iterator_tag());
                        return last;
                    }
                    else
                    {
                        corsac::reverse_impl(first, middle, bidirectional_iterator_tag());
                        return first;
                    }
                }
                */
        ...
        template <>
        struct rotate_helper<corsac::bidirectional_iterator_tag, true>
        {
            template <typename BidirectionalIterator>
            static BidirectionalIterator rotate_impl(BidirectionalIterator first, BidirectionalIterator middle, BidirectionalIterator last)
            {
        ...
        template <>
        struct rotate_helper<corsac::random_access_iterator_tag, false>
        {
        ...
        template <>
        struct rotate_helper<corsac::random_access_iterator_tag, true>
        {
            ...
            static RandomAccessIterator rotate_impl(RandomAccessIterator first, RandomAccessIterator middle, RandomAccessIterator last)
            {
                ...
                if((last - first) < 32) // For small ranges rotate_general_impl is faster.
                    return internal::rotate_general_impl(first, middle, last);
                return internal::rotate_helper<corsac::random_access_iterator_tag, false>::rotate_impl(first, middle, last);
            }
        };
    } // namespace internal
    ...
    ForwardIterator rotate(ForwardIterator first, ForwardIterator middle, ForwardIterator last)
    {
        if(middle != first)
        {
            if(middle != last)
            {
                ...
                return internal::rotate_helper<IC, corsac::is_trivially_move_assignable<value_type>::value || // This is the best way of telling if we can move types via memmove, but without a conforming C++11 compiler it usually returns false.
                                                   corsac::is_pod<value_type>::value                       || // This is a more conservative way of telling if we can move types via memmove, and most compilers support it, but it doesn't have as full of coverage as is_trivially_move_assignable.
                                                   corsac::is_scalar<value_type>::value>                      // This is the most conservative means and works with all compilers, but works only for scalars.
    ...
    {
        return corsac::copy(first, middle, corsac::copy(middle, last, result));
    }
    ...
    constexpr const T& clamp(const T& v, const T& lo, const T& hi, Compare comp)
    {
        ...
        return [&] { CORSAC_ASSERT(!comp(hi, lo)); }(),
    ...
    {
        return corsac::clamp(v, lo, hi, corsac::less<>());
    }
}

#endif //CORSAC_STL_ALGORITHM_H
