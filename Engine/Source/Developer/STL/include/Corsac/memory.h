 ...
 * utility.h
 *
 * Created by Falldot on 07.11.2021.
 * Copyright (c) 2021 Corsac. All rights reserved.
 */
#ifndef CORSAC_STL_MEMORY_H
#define CORSAC_STL_MEMORY_H
 ...
 *  Trait                                            Description
 * ---------------------------------------------------------------------------------------------------
 * === Неинициализированные операции:
 *    uninitialized_copy                        Копирует элементы из диапазона в неинициализированную область памяти.
 *    uninitialized_copy_n                      Создает копии из диапазона в неинициализированную область памяти.
 *    uninitialized_default_construct           Копирует элементы из диапазона в неинициализированную область памяти при инициализации по умолчанию.
 *    uninitialized_default_construct_n         Создает копии из диапазона в неинициализированную область памяти при инициализации по умолчанию.
 *    uninitialized_move
...
{
    /**
    * CORSAC_TEMP_DEFAULT_NAME
    *
    * Определяет имя контейнера по умолчанию при отсутствии имени, предоставленного пользователем.
    */
    #ifndef CORSAC_TEMP_DEFAULT_NAME
        #define CORSAC_TEMP_DEFAULT_NAME CORSAC_DEFAULT_NAME_PREFIX " temp" // Если пользователь что-то не отменяет, это «CORSAC temp».
    #endif

    /**
    * late_constructed
    *
    * Реализует тип интеллектуального указателя, который отделяет выделение памяти объекта от
    * конструкции объекта. Основной вариант использования - объявить глобальную переменную типа
    * late_construction, которая позволяет памяти быть глобальной, но конструктор выполняется в
    * какой-то момент после запуска main(), а не до main, что часто опасно для нетривиальных типов.
    *
    * Параметр шаблона autoConstruct определяет, будет ли объект автоматически создаваться по
    * умолчанию при первой ссылке или должен быть создан вручную при первом использовании
    * оператора или ->. autoConstruct удобен, но вызывает и -> немного медленнее и может
    * привести к построению в неудобное время.
    *
    * Параметр шаблона autoDestruct определяет, будет ли объект, если он создан, автоматически
    * разрушаться при вызове ~late_constructed () или должен быть разрушен вручную с помощью
    * вызова destruct().
    *
    * Хотя строительство может быть автоматическим или ручным, поддержка автоматического разрушения
    * присутствует всегда. Таким образом, вам ни в коем случае не требуется вручную вызывать
    * destruct. Однако вы можете безопасно вручную уничтожить объект в любое время до выполнения
    * деструктора late_constructed.
    *
    * Вы все еще можете использовать late_constructed после вызова destruct(), включая повторный
    * вызов construct() для восстановления экземпляра. destruct возвращает экземпляр
    * late_constructed в состояние, эквивалентное состоянию до вызова конструкции.
    *
    * Предостережение: хотя экземпляры late_constructed могут быть объявлены в глобальной области и
    * инициализированы до выполнения main(), в противном случае вы не можете использовать такие
    * глобально объявленные экземпляры до main с гарантированным поведением, если вы не можете
    * гарантировать, что экземпляр late_constructed сам создан до использования вами.
    *
    * Пример использования (демонстрирующий ручное построение):
    ...
    *     }
    *
    * Пример использования (демонстрирующий автоматическое построение):
    *     late_constructed<Widget, true> gWidget;
    *
    *     void main(){
    *         gWidget->SetValue(15);
    *         // Вы можете захотеть вызвать здесь destruct, но это не обязательно, если этого не требует тип Widget.
    *     }
    */
    ...
    public:
        ...
        using value_type   = T;
        using storage_type = corsac::aligned_storage_t<sizeof(value_type), corsac::alignment_of_v<value_type>>;

        late_constructed() noexcept       // В случае, если экземпляр late_constructed находится в глобальной области видимости, мы полагаемся на
                : mStorage(), mpValue(nullptr) {}  // компилятор, выполняющий этот конструктор или помещающий экземпляр в память с автоматическим обнулением при запуске.
        ...
        {
            if(!mpValue)
                mpValue = new (&mStorage) value_type(corsac::forward<Args>(args)...);
        }
        ...
        value_type& operator*() noexcept
        {
            ...
            CORSAC_ANALYSIS_ASSUME(mpValue);
            return *mpValue;
        }
    ...
    protected:
        storage_type mStorage; // Объявлен первым, потому что у него могут быть требования к настройкам, и он был бы более экономичным, если бы он был первым.
        value_type*  mpValue;
    };

    // Специализация, которая не создается автоматически по запросу.
    ...
    {
    public:
        using base_type = late_constructed<T, true, autoDestruct>;

        typename base_type::value_type& operator*() noexcept
        { CORSAC_ASSERT(base_type::mpValue); return *base_type::mpValue; }
    ...
    class raw_storage_iterator : public iterator<corsac::output_iterator_tag, void, void, void, void>
    {
    protected:
    ...
    public:
        ...
        {
            ::new(corsac::addressof(*mIterator)) T(value);
            return *this;
        }
    ...
    namespace internal
    {
            ...
            static ForwardIteratorDest do_move_start(ForwardIterator first, ForwardIterator last, ForwardIteratorDest dest)
            {
                typedef typename corsac::iterator_traits<ForwardIterator>::value_type value_type;
                                ...
                                for(; first != last; ++first, ++dest)
                                    ::new((void*)corsac::addressof(*dest)) value_type(*first);
                #if CORSAC_EXCEPTIONS_ENABLED
                                }
                                        catch(...)
                                        {
        ...
        template <>
        struct uninitialized_relocate_impl<true, corsac::random_access_iterator_tag>
        {
            template <typename T>
    ...
    {
        using IC                    = typename corsac::iterator_traits<ForwardIterator>::iterator_category;
        using value_type_input      = typename corsac::iterator_traits<ForwardIterator>::value_type;
        using value_type_output     = typename corsac::iterator_traits<ForwardIteratorDest>::value_type;

        const bool bHasTrivialMove = type_and<is_trivial<value_type_input>::value,
    ...
    inline ForwardIteratorDest uninitialized_relocate(ForwardIterator first, ForwardIterator last, ForwardIteratorDest dest)
    {
        ...
        corsac::uninitialized_relocate_commit(first, last, dest);
    ...
    namespace internal
    {
        ...
        {
            return corsac::copy(first, last, dest); // The copy() in turn will use memcpy for POD types.
        }
        ...
        inline ForwardIterator uninitialized_copy_impl(InputIterator first, InputIterator last, ForwardIterator dest, false_type)
        {
            ...
            ForwardIterator currentDest(dest);
                        ...
                        for(; first != last; ++first, ++currentDest)
                            ::new(static_cast<void*>(corsac::addressof(*currentDest))) value_type(*first);
            #if CORSAC_EXCEPTIONS_ENABLED
                        }
                                catch(...)
                                {
    ...
    * uninitialized_copy
    *
    * Копирует исходный диапазон в место назначения, создавая копирование
    * исходные значения (а не назначение назначения с исходными значениями).
    * Возвращает конец диапазона назначения (т. Е. Dest + (last - first)).
    *
    ...
    */
    ...
    inline ForwardIterator uninitialized_copy(InputIterator first, InputIterator last, ForwardIterator result)
    {
        ...
        return internal::uninitialized_copy_impl(first, last, result, corsac::is_trivial<value_type>());
    }
    ...
    *
    * Копирует счетные элементы из диапазона, начинающегося сначала, в неинициализированную область
    * памяти, начинающуюся с dest. Элементы в неинициализированной области создаются с помощью
    * конструктора копирования. Если во время инициализации возникает исключение, функция не имеет
    * окончательных эффектов.
    *
    * first:        Начало диапазона копируемых элементов.
    * dest:         Начало диапазона назначения.
    * return value: Итератор типа dest к элементу за последним скопированным элементом.
    */
    namespace internal
    {
            ...
            static ForwardIterator impl(InputIterator first, Count n, ForwardIterator dest)
            {
                ...
                ForwardIterator currentDest(dest);
                                ...
                                for(; n > 0; --n, ++first, ++currentDest)
                                    ::new((void*)(corsac::addressof(*currentDest))) value_type(*first);
                #if CORSAC_EXCEPTIONS_ENABLED
                                }
                                        catch(...)
                                        {
        ...
        struct uninitialized_copy_n_impl<InputIterator, Count, ForwardIterator, corsac::random_access_iterator_tag>
        {
            static inline ForwardIterator impl(InputIterator first, Count n, ForwardIterator dest)
            {
                return corsac::uninitialized_copy(first, first + n, dest);
            }
        };
    }
    ...
    */
    ...
    namespace internal
    {
        ...
        {
            return corsac::copy(first, last, dest); // The copy() in turn will use memcpy for is_trivially_copy_assignable (e.g. POD) types.
        }
        ...
        inline ForwardIterator uninitialized_move_impl(InputIterator first, InputIterator last, ForwardIterator dest, false_type)
        {
            ...
            #endif
                        for(; first != last; ++first, ++currentDest)
                            ::new((void*)corsac::addressof(*currentDest)) value_type(corsac::move(*first)); // If value_type has a move constructor then it will be used here.
            #if CORSAC_EXCEPTIONS_ENABLED
                        }
                                catch(...)
                                {
    ...
    {
        typedef typename corsac::iterator_traits<generic_iterator<Result, void> >::value_type value_type;
        const generic_iterator<Result, void> i(internal::uninitialized_move_impl(corsac::generic_iterator<First, void>(first), // generic_iterator makes a pointer act like an iterator.
                                                                                 corsac::generic_iterator<Last, void>(last),
                                                                                 corsac::generic_iterator<Result, void>(dest),
                                                                                 corsac::is_trivially_copy_assignable<value_type>())); // is_trivially_copy_assignable identifies if copy assignment would be as valid as move assignment, which means we have the opportunity to memcpy/memmove optimization.
        return i.base();
    }
    ...
    {
        return corsac::uninitialized_copy_n(corsac::make_move_iterator(first), n, dest);
    }
    ...
    *
    * Создает объекты в неинициализированном хранилище, обозначенном диапазоном [первый, последний) при инициализации по умолчанию.
    *
    * Инициализация по умолчанию:
    * если T - класс, вызывается конструктор по умолчанию; в противном случае инициализация не выполняется,
    * что приводит к неопределенным значениям.
    */
    template <typename ForwardIterator>
    ...
    *
    * Создает n объектов в неинициализированном хранилище, начиная с инициализации по умолчанию.
    */
    template <typename ForwardIterator, typename Count>
    ...
    namespace internal
    {
        ...
        #endif
                    for(; currentDest != last; ++currentDest)
                        ::new((void*)corsac::addressof(*currentDest)) value_type(value);
        #if CORSAC_EXCEPTIONS_ENABLED
                    }
                            catch(...)
                            {
    ...
    {
        typedef typename corsac::iterator_traits<ForwardIterator>::value_type value_type;
        internal::uninitialized_fill_impl(first, last, value, corsac::is_trivially_copy_assignable<value_type>());
    }
    ...
    {
        typedef typename corsac::iterator_traits<corsac::generic_iterator<T*, void> >::value_type value_type;
        internal::uninitialized_fill_impl(corsac::generic_iterator<T*, void>(first),
                                          corsac::generic_iterator<T*, void>(last), value,
                                          corsac::is_trivially_copy_assignable<value_type>());
    }
    ...
    */
    ...
    {
        const ForwardIterator mid(corsac::uninitialized_copy(first1, last1, first2));
        ...
        typedef typename corsac::iterator_traits<ForwardIterator>::value_type value_type;
                ...
                {
#endif
        corsac::uninitialized_fill(mid, last2, value);
#if CORSAC_EXCEPTIONS_ENABLED
        }
                catch(...)
                {
    ...
    inline void uninitialized_move_fill(InputIterator first1, InputIterator last1,
                                        ForwardIterator first2, ForwardIterator last2, const T& value)
    {
        const ForwardIterator mid(corsac::uninitialized_move(first1, last1, first2));
    ...
    {
        corsac::uninitialized_fill(result, mid, value);
        ...
        typedef typename corsac::iterator_traits<ForwardIterator>::value_type value_type;
                ...
                {
#endif
        return corsac::uninitialized_copy(first, last, mid);
#if CORSAC_EXCEPTIONS_ENABLED
        }
                catch(...)
                {
    ...
    uninitialized_fill_move(ForwardIterator result, ForwardIterator mid, const T& value, InputIterator first, InputIterator last)
    {
        ...
        typedef typename corsac::iterator_traits<ForwardIterator>::value_type value_type;
                ...
                {
#endif
        return corsac::uninitialized_move(first, last, mid);
#if CORSAC_EXCEPTIONS_ENABLED
        }
                catch(...)
                {
    ...
    {
        const ForwardIterator mid(corsac::uninitialized_copy(first1, last1, result));
        ...
        typedef typename corsac::iterator_traits<ForwardIterator>::value_type value_type;
                ...
                {
#endif
        return corsac::uninitialized_copy(first2, last2, mid);
#if CORSAC_EXCEPTIONS_ENABLED
        }
                catch(...)
                {
    ...
    */
    ...
    {
        using value_type = typename corsac::iterator_traits<ForwardIterator>::value_type;
        destruct_impl(first, last, corsac::is_trivially_destructible<value_type>());
    }
    ...
    /// is a no-op.
    ///
    /// This is the same as corsac::destruct but we included for C++17 compliance.
    ///
    ...
    {
        for (; first != last; ++first)
            corsac::destroy_at(corsac::addressof(*first));
    }
    ...
    ///     void*  p  = buffer;
    ///     void*  p1 = corsac::align(16,  3, p, space); p = (char*)p +  3; space -=  3;
    ///     void*  p2 = corsac::align(32, 78, p, space); p = (char*)p + 78; space -= 78;
    ///     void*  p3 = corsac::align(64,  9, p, space); p = (char*)p +  9; space -=  9;
    ...
    ///     size_t space = sizeof(buffer);
    ///     void*  p  = buffer;
    ///     void*  p1 = corsac::align_advance(16,  3, p, space, &p, &space); // p is advanced and space reduced accordingly.
    ///     void*  p2 = corsac::align_advance(32, 78, p, space, &p, &space);
    ///     void*  p3 = corsac::align_advance(64,  9, p, space, &p, &space);
    ///     void*  p4 = corsac::align_advance(16, 33, p, space);
    ...
    private:
        ...
        template <typename U>
        static corsac::yes_type test(typename U::allocator_type* = NULL);

    public:
        static const bool value = sizeof(test<T>(NULL)) == sizeof(corsac::yes_type);
    };


    template <typename T, typename Allocator, bool = has_allocator_type_helper<T>::value>
    struct uses_allocator_impl
            : public integral_constant<bool, corsac::is_convertible<Allocator, typename T::allocator_type>::value>
    {};
    ...
    namespace internal
    {
        ...
        {
        private:
            template <typename U> static corsac::no_type  test(...);
            template <typename U> static corsac::yes_type test(typename U::element_type* = 0);
        public:
            static const bool value = sizeof(test<Pointer>(0)) == sizeof(corsac::yes_type);
        };
        ...
        private:
            template <typename U> static corsac::no_type  test(...);
            template <typename U> static corsac::yes_type test(typename U::difference_type* = 0);
        public:
            static const bool value = sizeof((test<Pointer>(0))) == sizeof(corsac::yes_type);
        };
        ...
        // its own type), but we don't currently use rebind in CORSAC (as we have a different allocator
    ...
    struct pointer_traits
    {
        ...
        using element_type      = typename internal::pointer_element_type<pointer>::type;
        using difference_type   = typename internal::pointer_difference_type<pointer>::type;

        template <typename U>
        using rebind = typename internal::pointer_rebind<pointer, U>::type;

    public:
        static pointer pointer_to(typename corsac::conditional<corsac::is_void<element_type>::value, void, element_type>::type& r) // 20.6.3.2: if element_type is (possibly cv-qualified) void, the type of r is unspecified; otherwise, it is T&.
    ...
    public:
        static pointer pointer_to(typename corsac::conditional<corsac::is_void<element_type>::value, void, element_type>::type& r) noexcept
        { return corsac::addressof(r); } // 20.6.3.2: if element_type is (possibly cv-qualified) void, the type of r is unspecified; otherwise, it is T&.
    };
