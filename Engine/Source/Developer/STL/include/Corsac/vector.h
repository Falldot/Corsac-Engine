...
namespace corsac
{
    ...
    */
    ...
    public:
        ...
        const_pointer get(size_type n) const;
    ...
    {
        if(mpBegin)
            CORSAC_Free(internalAllocator(), mpBegin, static_cast<size_t>(internalCapacityPtr() - mpBegin) * sizeof(T));
    }
    ...
    template <typename T, typename Allocator>
    inline typename vector<T, Allocator>::pointer
    vector<T, Allocator>::get(size_type n)
    {
    ...
    template <typename T, typename Allocator>
    inline typename vector<T, Allocator>::const_pointer
    vector<T, Allocator>::get(size_type n) const
    {
        if(CORSAC_UNLIKELY(n >= (static_cast<size_type>(mpEnd - mpBegin))))
            return nullptr;
        return (mpBegin + n);
    ...
    {
        #if CORSAC_ASSERT_ENABLED && CORSAC_EMPTY_REFERENCE_ASSERT_ENABLED
            if (CORSAC_UNLIKELY((mpBegin == nullptr) || (mpEnd <= mpBegin))) // Мы не разрешаем пользователю ссылаться на пустой контейнер.
                {CORSAC_FAIL_MSG("vector::front -- empty vector")}
        #else
            // Мы позволяем пользователю ссылаться на пустой контейнер.
        #endif
